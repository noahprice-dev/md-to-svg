...
pub fn create_default_test_font_system() -> FontSystem {
    ...
    db.load_font_file(get_normalized_fixture_path(&["tests", "fonts", "NotoSans-Bold.ttf"]))
        .expect("Bold test font should load successfully.");
    db.load_font_file(get_normalized_fixture_path(&["tests", "fonts", "NotoSans-BoldItalic.ttf"]))
        .expect("BoldItalic test font should load successfully.");
    db.load_font_file(get_normalized_fixture_path(&["tests", "fonts", "NotoSans-Italic.ttf"]))
        .expect("Italic test font should load successfully.");
    db.load_font_file(get_normalized_fixture_path(&["tests", "fonts", "NotoSans-Regular.ttf"]))
...
pub fn get_normalized_fixture_path(sub_paths: &[&str]) -> PathBuf {
